...
Stmt makeStmt(std::string kernel)
{
    ...
    //===================================================================================================================
    // some uncertain changes by LYC
    makeTree(kernel.substr(0, i));
    std::vector<Expr> kroneckerDelta;
    kernel = kernel.substr(i);
    while (kernel[0] == '!') {
        size_t tpos = kernel.find(']');
        std::string constraintStr = kernel.substr(2, tpos - 2);
        std::string indStr1 = constraintStr.substr(0, constraintStr.find(','));
        std::string indStr2 = constraintStr.substr(constraintStr.find(',') + 1);
        Expr ind1 = makeIndex(indStr1, INF);
        Expr ind2 = makeIndex(indStr2, INF);
        kroneckerDelta.push_back(Compare::make(index_type, CompareOpType::EQ, ind1, ind2));
        kernel.erase(0, tpos + 1);
    }
    i = 0;
    // that' ok
    ...
    //tmp value set 0
    if (kernel[i] == '+') {
        tmpStmt = Move::make(TMP, LHS,MoveType::MemToMem);
        i += 3;
    }
    else {
    ...
    for(i; i < length; i++)
    {
        ...
        if(flag == 0)
        {
            if(kernel[i] == '+' || kernel[i] == '-' || kernel[i] == ';')
            {
                ...
                for (auto i : kroneckerDelta) {
                    tmpStmt = IfThenElse::make(i, tmpStmt, emptyStmt);
                }
                // loop nest
                loop.clear();
                //std::cout<<"innerloop: ";
